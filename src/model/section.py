...
class PageSettings:
    ...
    def validate(self) -> None:
        ...
        if self.margins.left + self.margins.right >= self.width:
            raise ValueError(
                f'Сумма горизонтальных полей ({self.margins.left:.3f}" + '
                f'{self.margins.right:.3f}") превышает ширину страницы '
                f'({self.width:.3f}")'
            )
    ...
    # Alias для совместимости с document.PageSettings
    get_printable_width_inches = get_printable_width
    ...
    # Alias для совместимости с document.PageSettings
    get_printable_height_inches = get_printable_height
