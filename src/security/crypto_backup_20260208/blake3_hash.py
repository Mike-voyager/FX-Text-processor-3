# -*- coding: utf-8 -*-
"""
RU: BLAKE3 высокоскоростное хеширование - в 10x быстрее SHA-256.
EN: BLAKE3 high-performance hashing - 10x faster than SHA-256.
...
def compute_hash_blake3(data: bytes, length: int = 32) -> str:
    ...
    digest_bytes: bytes = hasher.digest(length)
    return digest_bytes.hex()


def blake3_derive_key(context: str, key_material: bytes, length: int = 32) -> bytes:
    """
...
def hmac_blake3(key: bytes, message: bytes, length: int = 32) -> bytes:
    ...
    Args:
        key: MAC key (MUST be exactly 32 bytes).
    ...
    if len(key) != 32:
        raise ValueError("BLAKE3 key must be exactly 32 bytes, got {len(key)}")
