    ...
    def generate_ecdsa(cls, curve: str = "P-256") -> PIVKeyPair:
        ...
        if curve not in PIV_EC_CURVES:
            raise CryptoKeyError(
                f"PIV EC curve must be one of {list(PIV_EC_CURVES.keys())}"
            )

        private_key = ec.generate_private_key(
            PIV_EC_CURVES[curve], backend=default_backend()
        )
...
class OpenPGPKeyPair:
    ...
    def generate_ecdsa(cls, curve: str = "P-256") -> OpenPGPKeyPair:
        ...
        private_key = ec.generate_private_key(
            OPENPGP_EC_CURVES[curve], backend=default_backend()
        )
    ...
    def sign(self, data: bytes, hash_algo: str = "SHA256") -> bytes:
        ...
        if isinstance(self._private_key, rsa.RSAPrivateKey):
            return self._private_key.sign(
                data,
                padding.PSS(
                    mgf=padding.MGF1(hash_obj), salt_length=padding.PSS.MAX_LENGTH
                ),
                hash_obj,
            )

        elif isinstance(self._private_key, ec.EllipticCurvePrivateKey):
            return self._private_key.sign(data, ec.ECDSA(hash_obj))
    ...
    def verify(self, data: bytes, signature: bytes, hash_algo: str = "SHA256") -> bool:
        ...
        try:
            if isinstance(self._public_key, rsa.RSAPublicKey):
                ...
                self._public_key.verify(signature, data, ec.ECDSA(hash_obj))
                return True
...
class CamelliaGCM:
    ...
    def encrypt(
        ...
        cipher = Cipher(
            algorithms.Camellia(key), modes.GCM(nonce), backend=default_backend()
        )
    ...
    def decrypt(
        self, key: bytes, nonce: bytes, combined: bytes, aad: Optional[bytes] = None
    ) -> bytes:
        """
        ...
        cipher = Cipher(
            algorithms.Camellia(key), modes.GCM(nonce, tag), backend=default_backend()
        )
...
class TwofishCTR:
    ...
    def __init__(self):
        ...
        try:
            ...
            raise ImportError("Twofish not available. Install: pip install twofish")
    ...
    def decrypt(self, key: bytes, nonce: bytes, ciphertext: bytes) -> bytes:
        ...
        cipher = Cipher(
            algorithms.TripleDES(key), modes.CTR(nonce), backend=default_backend()
        )
